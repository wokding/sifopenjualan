...
class Auth extends CI_Controller
{
        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    // Clear all message shown flags before login
                    $session_data = $this->session->userdata();
                    foreach($session_data as $key => $value) {
                        if(strpos($key, 'msg_shown_') === 0) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Welcome back! Login successful.</div>');
                    if ($user['role_id'] == 1) {
                        redirect('admin');
                    } else {
                        redirect('user');
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            Congratulations! Your account has been created. Please wait for admin activation!</div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_pass' => '2760B692E6A8355D2878520F1BE66C5E2C77',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n",
    ...
    {
        // Clear all message shown flags before logout
        $session_data = $this->session->userdata();
        foreach($session_data as $key => $value) {
            if(strpos($key, 'msg_shown_') === 0) {
                $this->session->unset_userdata($key);
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
        Your password has been reset to: <strong>password123</strong>. Please login and change it immediately!</div>');
                redirect('auth');
            } else {
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
